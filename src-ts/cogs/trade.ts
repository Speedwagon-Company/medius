...
export class TradeCog extends Cog {
  @Cog.command("starttrade", "Start a mediated crypto trade")
  @Cog.argument("user", ArgumentType.User, "The user to trade with", true)
  async startTrade(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      const selectedCoin = await this.askCoin(interaction);
      const confirmed = await this.askConfirmation(interaction, target, selectedCoin);

      if (!confirmed) {
        return;
      }

      const channel = await this.createTicketChannel(interaction, target);
      await createTrade(initiator.id, target.id);

      const roles = await this.askTradeRoles(channel, [initiator, target], selectedCoin);
      if (!roles) {
        await delay(10_000);
      ...
      const senderWallet = await this.getWalletInTries(channel, sender, SEND_WALLET_TRIES);
      ...
      const receiverWallet = await this.getWalletInTries(channel, receiver, SEND_WALLET_TRIES);
      ...
      if (receipt.status === "success") {
        ...
        const releaseConfirmed = await this.askReleaseMoney(
          channel,
          transactionMessage,
          receiver,
          selectedCoin,
        ...
        if (!releaseConfirmed) {
          await this.handleCancelMoney(value, senderWallet, channel);
          return;
        }

        await this.handleConfirmMoney(value, receiverWallet, channel);
        return;
      }
  ...
  private async createTicketChannel(
    ...
    const channelName = `trade-${currentUser.user.username}-${user.user.username}`.toLowerCase().replace(/[^a-z0-9-]/g, "-");
  ...
  private async askCoin(interaction: ChatInputCommandInteraction<"cached">): Promise<string> {
  ...
  private async askTradeRoles(
    ...
    const message = await channel.send({
      embeds: [await this.buildRolesEmbed(selectedCoin, roles)],
      components: [this.buildRoleButtons(receiverId, senderId, cancelId, roles)],
    });
  ...
  private async buildRolesEmbed(selectedCoin: string, roles: RoleSelection): Promise<EmbedBuilder> {
    return createSuccessEmbed(
      "Select your role",
      `**Coin:** ${selectedCoin}\n**Sender:** ${roles[TradeRole.Sender]?.toString() ?? "Not selected"}\n**Receiver:** ${
        roles[TradeRole.Receiver]?.toString() ?? "Not selected"
      }`,
    );
  }

  private async getWalletInTries(channel: TextChannel, member: GuildMember, tries: number): Promise<string> {
...
async function createTransaction(payload: {
  ...
  // TODO: port/import the project's DB create_transaction implementation.
  return {
    id: Date.now(),
...
async function updateTransactionStatus(transactionId: number, status: string): Promise<void> {
  // TODO: port/import the project's DB update_transaction_status implementation.
  void transactionId;
  void status;
}

async function sendTransactionLog(guild: Guild, embed: EmbedBuilder): Promise<void> {
  // TODO: port/import the project's transaction log sender and configured log channel id.
  void guild;
  void embed;
}
